...
with open('./preprocess/codes_regexp_patterns.json', 'r', encoding='utf-8') as f:
    codes_regexp_patterns = json.load(f)
with open('./preprocess/na_levels_regexp_patterns.json', 'r', encoding='utf-8') as f:
...
class LegalDocument:
    ...
    def get_na_level(self, level_pattern, text):
        ...
        if ranged_numbers is not None:
            ...
            na_levels_numbers = re.findall('[\d\.]+', na_level_part_without_range)
    ...
    @property
    def na_list(self):
        return [na for na in self.__na_list]
